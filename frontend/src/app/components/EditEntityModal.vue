...
const editedName = ref(props.entityName)
const selectedRoomId = ref(props.roomId ?? '')
const validationError = ref('')
const isRoomMenuOpen = ref(false)
const roomSelectorRef = ref<HTMLElement | null>(null)

const hasRoomSelector = computed(() => props.entityType === 'device' && props.rooms.length > 0)

watch(() => props.entityName, (nextName) => {
...
function handleSubmit() {
    ...
    if (!trimmedName) {
        validationError.value = props.entityType === 'device' 
            ? 'Ingresá un nombre para el dispositivo.'
            : 'Ingresá un nombre para la rutina.'
        return
    }

    const payload: { name: string; roomId?: string } = { name: trimmedName }
    if (hasRoomSelector.value && selectedRoomId.value) {
        if (!props.rooms.some(room => room.id === selectedRoomId.value)) {
            validationError.value = 'Seleccioná una habitación para el dispositivo.'
            return
        }
        payload.roomId = selectedRoomId.value
    }
            ...
            <div class="modal">
                <div class="modal__header">
                    <h2 class="modal__title">
                        {{ entityType === 'device' ? 'Editar dispositivo' : 'Editar rutina' }}
                    </h2>
                    <button
                        class="modal__close"
                        type="button"
                ...
                <form class="modal__form" @submit.prevent="handleSubmit" novalidate>
                    <div class="field">
                        <input
                            ...
                            maxlength="25"
                        />
                        <label for="entity-edit-name" class="field__label">
                            {{ entityType === 'device' ? 'Nombre del dispositivo' : 'Nombre de la rutina' }}
                        </label>
                    </div>

                    <div v-if="hasRoomSelector" class="field field--room">
